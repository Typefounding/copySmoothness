...
class CopySmoothness:
    ...
    def __init__(self):
        ...
        self.w.sourceFont = PopUpButton((15, 41, -20, 20), [f.info.familyName + ' ' + f.info.styleName for f in self.sourceFontList], callback=self.sourceCallback)
        self.w.desTitle = TextBox((15, 80, 200, 20), "Destination Fonts:")
        self.w.destinationFonts = FontList((15, 100, -15, -105), self.destinationFontList, selectionCallback=self.desCallback)
        self.w.markGlyphs = CheckBox((20, -84, 100, 22), "Mark Glyphs", callback=self.markCallback, value=self.doMarkGlyphs)
        self.w.copyButton = Button((-140, -40, 120, 20), 'Copy Smoothness', callback=self.copyCallback)
    ...
    def markCallback(self, sender):
        ...
        if self.doMarkGlyphs == 1:
            self.w.colorWell = ColorWell((-220, -85, 100, 23), callback=self.colorCallback, color=self.mark)
        else:
